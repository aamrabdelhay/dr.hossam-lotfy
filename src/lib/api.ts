...
export async function readJson<T>(req: Request, schema: ZodType<T>): Promise<T> {
  ...
  const result = schema.safeParse(raw); if (!result.success) { const first = result.error.issues[0]; const path = (first?.path ?? []).map(String).join('.'); const msg = first?.message ?? 'بيانات غير صالحة'; throw new ApiError(400, `خطأ في المدخلات${path ? ` (${path})` : ''}: ${msg}`); }
  return result.data;
}
