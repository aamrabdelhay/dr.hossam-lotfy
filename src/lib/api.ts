...
export async function user(): Promise<SessionUser | null> { try { return await getCurrentUser(); } catch { return null; } }
...
export async function requireAdmin(): Promise<AdminSession> { const u = await requireStaff(); if (u.userRole !== 'SUPER_ADMIN' && u.userRole !== 'ADMIN') throw new ApiError(403, 'هذا الإجراء يتطلب صلاحية المدير العام'); return u; }
export async function requireLawyer(): Promise<Extract<SessionUser, { role: 'lawyer' }>> { const u = await user(); if (!u || u.role !== 'lawyer') throw new ApiError(401, 'يجب تسجيل الدخول كمحامي لإتمام هذا الإجراء'); return u; }
...
export async function readJson<T>(req: NextRequest, schema: ZodType<T>): Promise<T> {
  let raw: unknown; try { raw = await req.json(); } catch { throw new ApiError(400, 'طلب غير صالح: تعذر قراءة البيانات'); }
  const result = schema.safeParse(raw); if (!result.success) { const first = result.error.issues[0]; const path = (first?.path ?? []).map(String).join('.'); const msg = (first?.message ?? 'بيانات غير صالحة').replace(/"([a-zA-Z_]+)"/g, '$1'); throw new ApiError(400, `خطأ في المدخلات${path ? ` (${path})` : ''}: ${msg}`); }
  return result.data;
}

type RouteContext = { params?: unknown };
type RouteHandler = (req: Request, context?: RouteContext) => Promise<NextResponse>;
