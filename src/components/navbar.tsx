...
import { Search, Landmark, Scale, KeyRound, LogOut, Menu, X, CalendarClock, UserCircle2, Archive, ChevronDown } from 'lucide-react';
...
export function Navbar({ lawyers, locations, session, unread }: NavbarProps) {
  ...
  const navLinkStyle = (active: boolean) => cn(
    ...
    active ? 'bg-navy-900 text-white shadow-soft' : 'text-navy-500 hover:bg-navy-900/[0.06] hover:text-navy-900',
  );
  ...
  const locationList = (
    <div className="w-72 overflow-hidden rounded-2xl border border-navy-100 bg-white shadow-lift ring-1 ring-navy-950/5">
      ...
      <ul className="max-h-64 overflow-y-auto overscroll-contain py-1.5 [scrollbar-gutter:stable]">
        {courts.map((l) => <li key={l.id}><Link href={`/locations/${l.slug}`} className="flex items-center gap-2.5 px-4 py-2 text-[13px] font-medium text-[#1D2433] transition-colors hover:bg-ivory-100" style={{ fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}><Landmark size={14} className="text-gold-600" />{l.name}</Link></li>)}
      </ul>
      ...
      <ul className="max-h-48 overflow-y-auto overscroll-contain py-1.5 [scrollbar-gutter:stable]">
        {otherLocations.map((l) => <li key={l.id}><Link href={`/locations/${l.slug}`} className="flex items-center gap-2.5 px-4 py-2 text-[13px] font-medium text-[#1D2433] transition-colors hover:bg-ivory-100" style={{ fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}><Scale size={14} className="text-[#5B6B84]" />{l.name}<span className="ms-auto rounded-full bg-ivory-100 px-2 py-0.5 text-[10px] font-bold text-[#5B6B84]">{LOCATION_TYPE_LABEL[l.type]}</span></Link></li>)}
      </ul>
      <Link href="/locations" className="block border-t border-navy-100 bg-ivory-50 px-4 py-2.5 text-center text-[11px] font-bold tracking-[0.5px] uppercase text-gold-700 transition-colors hover:bg-gold-500/10" style={{ fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}>عرض كل المحاكم والأماكن</Link>
    </div>
  );
  ...
  const lawyerList = (
    <div className="w-72 overflow-hidden rounded-2xl border border-navy-100 bg-white shadow-lift ring-1 ring-navy-950/5">
      <ul className="max-h-80 overflow-y-auto overscroll-contain py-1.5 [scrollbar-gutter:stable]">
        {[...lawyers].sort((a, b) => Number(b.isPrincipal) - Number(a.isPrincipal)).map((l) => <li key={l.id} className={cn(l.isPrincipal && 'bg-gold-500/[0.07]')}><Link href={`/lawyers/${l.slug}`} className="flex items-center gap-3 px-4 py-2 transition-colors hover:bg-ivory-100"><Avatar name={l.name} src={l.photo} size={28} /><span className="min-w-0"><span className="block truncate text-[13px] font-medium text-[#1D2433]" style={{ fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}>{l.name}</span><span className="block text-[10px] font-semibold tracking-[0.5px] uppercase text-[#5B6B84]" style={{ fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}>{TITLE_LABEL[l.title]}</span></span></Link></li>)}
      </ul>
      <Link href="/lawyers" className="block border-t border-navy-100 bg-ivory-50 px-4 py-2.5 text-center text-[11px] font-bold tracking-[0.5px] uppercase text-gold-700 transition-colors hover:bg-gold-500/10" style={{ fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}>عرض كل المحامين</Link>
    </div>
  );

  const loginLink = <Link href="/auth" prefetch={false} className="flex items-center gap-1.5 rounded-full px-2.5 py-1.5 text-[10px] font-bold tracking-[1.5px] uppercase text-navy-400 transition-all hover:scale-[1.03] hover:bg-navy-900/[0.06] hover:text-navy-900 active:scale-[0.98]" style={{ fontFamily: 'IBM Plex Sans Arabic, sans-serif' }} title="تسجيل الدخول"><KeyRound size={12} /><span className="hidden sm:inline">تسجيل دخول</span></Link>;
    ...
    <header className="glass-light sticky top-0 z-50 border-b border-navy-100 text-navy-950 shadow-[0_4px_24px_-16px_rgba(5,8,15,0.25)]" ref={wrapRef}>
      ...
      <div className="relative mx-auto flex h-[68px] max-w-[1440px] items-center gap-4 px-5 sm:px-6">
        ...
        <nav className="hidden items-center gap-1 lg:flex">
          ...
          <Link href="/lawyer-guide" className={navLinkStyle(isActive('/lawyer-guide'))}>دليل المحامي</Link>
        </nav>
        <div className="ms-auto flex items-center gap-2 lg:ms-4">
          ...
          {!session ? loginLink : session.role === 'admin' ? (
            ...
            <div className="flex items-center gap-2">
              <Link href={`/lawyers/${session.slug}`} className="flex items-center gap-2 rounded-full px-2.5 py-1.5 text-[11px] font-semibold tracking-[1px] text-navy-600 transition-all hover:scale-[1.02] hover:bg-navy-900/[0.06] hover:text-navy-950 active:scale-[0.98]" style={{ fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}><UserCircle2 size={14} className="text-gold-600" /><span className="hidden sm:inline">{session.name}</span></Link>
              <button onClick={logout} className="rounded-full p-2 text-navy-300 transition-all hover:scale-110 hover:bg-red-600/10 hover:text-red-600 active:scale-95" aria-label="تسجيل الخروج"><LogOut size={14} /></button>
            </div>
          )}
        </div>
      </div>
      {mobileOpen && (
        <div className="max-h-[calc(100dvh-68px)] overflow-y-auto overscroll-contain border-t border-navy-100 bg-white/95 backdrop-blur-xl lg:hidden [scrollbar-gutter:stable]"><div className="space-y-4 px-5 py-5"><form onSubmit={doSearch} className="flex items-center gap-2"><div className="relative flex-1"><Search size={13} className="absolute start-3.5 top-1/2 -translate-y-1/2 text-navy-300" /><input value={q} onChange={(e) => setQ(e.target.value)} placeholder="ابحث..." className="h-10 w-full rounded-full border border-navy-200/80 bg-white ps-9 pe-3 text-[12px] text-navy-900 shadow-soft transition-all placeholder:text-navy-300 focus:border-gold-500/60 focus:outline-none focus:ring-2 focus:ring-gold-500/20" style={{ fontFamily: 'IBM Plex Sans Arabic, sans-serif' }} /></div><button type="submit" className="h-10 shrink-0 rounded-full bg-gradient-to-b from-gold-400 to-gold-500 px-4 text-[11px] font-bold text-[#0A101D] transition-all hover:scale-[1.03] active:scale-[0.98]" style={{ fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}>بحث</button></form><div className="grid grid-cols-1 gap-1"><Link href="/locations" className="flex items-center gap-3 rounded-xl px-3 py-3 text-[12px] font-semibold text-navy-600 transition-all hover:scale-[1.01] hover:bg-ivory-100 hover:text-navy-950" style={{ fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}><Landmark size={15} className="text-gold-400/70" /> المحاكم والأماكن</Link><Link href="/lawyers" className="flex items-center gap-3 rounded-xl px-3 py-3 text-[12px] font-semibold text-navy-600 transition-all hover:scale-[1.01] hover:bg-ivory-100 hover:text-navy-950" style={{ fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}><Scale size={15} className="text-gold-400/70" /> المحامون</Link><Link href="/calendar" className="flex items-center gap-3 rounded-xl px-3 py-3 text-[12px] font-semibold text-navy-600 transition-all hover:scale-[1.01] hover:bg-ivory-100 hover:text-navy-950" style={{ fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}><CalendarClock size={15} className="text-gold-400/70" /> التقويم</Link><Link href="/search" className="flex items-center gap-3 rounded-xl px-3 py-3 text-[12px] font-semibold text-navy-600 transition-all hover:scale-[1.01] hover:bg-ivory-100 hover:text-navy-950" style={{ fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}><Search size={15} className="text-gold-400/70" /> البحث</Link><Link href="/lawyer-guide" className="flex items-center gap-3 rounded-xl px-3 py-3 text-[12px] font-semibold text-navy-600 transition-all hover:scale-[1.01] hover:bg-ivory-100 hover:text-navy-950" style={{ fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}><Landmark size={15} className="text-gold-400/70" /> دليل المحامي</Link><Link href="/cases/archive" className="flex items-center gap-3 rounded-xl px-3 py-3 text-[12px] font-semibold text-navy-600 transition-all hover:scale-[1.01] hover:bg-ivory-100 hover:text-navy-950" style={{ fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}><Archive size={15} className="text-gold-400/70" /> أرشيف القضايا</Link>{session && <button onClick={logout} className="flex items-center gap-3 rounded-xl px-3 py-3 text-start text-[12px] font-semibold text-red-600 transition-all hover:scale-[1.01] hover:bg-red-600/10" style={{ fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}><LogOut size={15} /> تسجيل الخروج</button>}{!session && <Link href="/auth" prefetch={false} className="flex items-center gap-3 rounded-xl px-3 py-3 text-[12px] font-semibold text-navy-400 transition-all hover:scale-[1.01] hover:bg-ivory-100 hover:text-navy-950" style={{ fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}><KeyRound size={15} /> تسجيل دخول</Link>}</div></div></div>
      )}
    </header>
  );
}
