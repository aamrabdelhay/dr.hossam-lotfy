...
export function Navbar({ lawyers, locations, session, unread }: NavbarProps) {
  ...
  return (
    <header ref={wrapRef} className="site-floating-nav glass-light fixed inset-x-0 top-2 z-50 mx-auto w-[calc(100%-0.75rem)] max-w-[1440px] overflow-x-hidden overflow-y-visible border border-white/70 text-navy-950 sm:top-3 sm:w-[calc(100%-2rem)] lg:top-4 lg:w-[calc(100%-3rem)]">
      <span aria-hidden className="gold-hairline pointer-events-none absolute inset-x-5 bottom-0 opacity-50 sm:inset-x-8" />
      <div className="relative mx-auto flex min-h-[58px] w-full items-center gap-1 px-2 py-2 sm:min-h-[64px] sm:gap-2 sm:px-4 sm:py-0 lg:gap-3 lg:px-6">
        ...
        <Link href="/" className="flex min-w-0 shrink items-center gap-2 sm:gap-3"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-gold-400 to-gold-600 text-[13px] font-semibold text-white shadow-glow-gold sm:h-10 sm:w-10 sm:text-[15px]">HL</span><span className="flex min-w-0 max-w-[104px] flex-col items-center text-center sm:max-w-[130px] md:max-w-none"><span className="block max-w-full truncate text-[10px] tracking-[0.5px] text-navy-950 sm:text-[12px] sm:tracking-[1px] md:text-[15px] md:tracking-[2px]">Dr.Hossam Loutfi</span><span className="block text-[7px] font-bold tracking-[1.5px] text-gold-600 uppercase sm:text-[9px] sm:tracking-[3px]">LAW FIRM</span></span></Link>
        ...
        <div className="ms-auto flex min-w-0 shrink-0 items-center gap-1 sm:gap-2">{session && <NotificationBell unread={unread} />}{!session ? loginLink : isAdmin ? <div className="flex items-center gap-1 sm:gap-2"><Link href="/admin" className="flex shrink-0 items-center gap-1.5 rounded-full px-2 py-1.5 text-[11px] font-semibold text-navy-600"><LayoutDashboard size={14} className="text-gold-600" /><span className="hidden sm:inline">الإدارة</span></Link><Link href="/cases/archive" className="hidden shrink-0 items-center gap-1.5 rounded-full px-2 py-1.5 text-[11px] font-semibold text-navy-600 sm:flex"><Archive size={14} className="text-gold-600" />أرشيف القضايا</Link><button onClick={logout} className="flex shrink-0 items-center gap-1.5 rounded-lg border border-red-200 bg-red-50 px-2 py-1.5 text-[11px] font-bold text-red-600 transition-colors hover:border-red-300 hover:bg-red-100"><LogOut size={14} /><span className="hidden sm:inline">تسجيل الخروج</span></button></div> : <div className="flex items-center gap-1"><Link href={`/lawyers/${session.slug}`} className="flex min-w-0 items-center gap-2 rounded-full px-2 py-1.5 text-[11px] font-semibold text-navy-600"><UserCircle2 size={14} className="shrink-0 text-gold-600" /><span className="hidden max-w-[120px] truncate sm:inline">{session.name}</span></Link><button onClick={logout} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-red-200 bg-red-50 text-red-600 transition-colors hover:border-red-300 hover:bg-red-100" aria-label="تسجيل الخروج"><LogOut size={14} /></button></div>}</div>
      </div>
      {mobileOpen && <div className="border-t border-white/50 bg-white/60 backdrop-blur-2xl lg:hidden"><div className="max-h-[calc(100dvh-78px)] space-y-4 overflow-x-hidden overflow-y-auto px-3 py-4 sm:px-5 sm:py-5"><form onSubmit={doSearch} className="flex items-center gap-2"><div className="relative min-w-0 flex-1"><Search size={13} className="absolute start-3.5 top-1/2 -translate-y-1/2 text-navy-300" /><input value={q} onChange={(e) => setQ(e.target.value)} placeholder="ابحث..." className="h-10 w-full rounded-full border border-white/70 bg-white/70 ps-9 pe-3 text-[12px] backdrop-blur-xl" /></div></form><div className="grid gap-2"><Link href="/locations" className={cn(navLinkStyle(isActive('/locations')), 'w-full')}>المحاكم والجهات الحكومية</Link><Link href="/lawyers" className={cn(navLinkStyle(isActive('/lawyers')), 'w-full')}>المحامون</Link><Link href="/calendar" className={cn(navLinkStyle(isActive('/calendar')), 'w-full')}>التقويم</Link><Link href="/search" className={cn(navLinkStyle(isActive('/search')), 'w-full')}>البحث</Link>{isAdmin && <><Link href="/admin" className={cn(navLinkStyle(isActive('/admin')), 'w-full')}>الإدارة</Link><Link href="/cases/archive" className={cn(navLinkStyle(isActive('/cases/archive')), 'w-full')}>أرشيف القضايا</Link></>}{!session ? loginLink : <button onClick={logout} className={cn(navLinkStyle(false), 'w-full border border-red-200 bg-red-50 text-red-600 hover:bg-red-100')}>تسجيل الخروج</button>}</div></div></div>}
    </header>
  );
}
