...
type NavbarProps = {
  ...
  session: { role: 'admin' | 'lawyer'; name: string; slug?: string; photo?: string | null; isAdmin?: boolean } | null;
  unread: number;
};
...
export function Navbar({ lawyers, locations, session, unread }: NavbarProps) {
  ...
  const isActive = (href: string) => {
    if (href === '/lawyers') return pathname?.startsWith('/lawyers') ?? false;
    if (href === '/locations') return pathname?.startsWith('/locations') ?? false;
