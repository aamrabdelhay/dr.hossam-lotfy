...
export function TaskCreator({ open, onClose, locations, lawyers, cases, defaultDate, onCreated }: {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!locationId || lawyerIds.length === 0) { toastError('المحكمة أو الجهة والمحامي مطلوبان.'); return; }
    setBusy(true); setProgress(0);
    let ok = 0; let failed = 0; let firstError = '';
    for (const lawyerId of lawyerIds) {
      try {
        const res = await fetch('/api/tasks', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ locationId, caseName: selectedCase?.name || undefined, caseNumber: selectedCase?.number || undefined, clientName: clientName.trim() || undefined, description: description.trim() || undefined, notes: notes.trim() || undefined, scheduledDate: date || undefined, scheduledTime: time || undefined, lawyerIds: [lawyerId] }) });
        if (res.ok) ok += 1; else { failed += 1; if (!firstError) { const d = await res.json().catch(() => null as { error?: string } | null); firstError = d?.error || `فشل الطلب (${res.status})`; } }
      } catch (err) { failed += 1; if (!firstError) firstError = err instanceof Error ? err.message : 'تعذر الاتصال بالخادم'; }
      setProgress(ok + failed);
    }
    setBusy(false);
    if (ok > 0) { setCreatedCount((v) => v + ok); toastSuccess(failed === 0 ? (ok > 1 ? `تم نشر ${ok} تكليفات ✓` : 'تم نشر التكليف ✓') : `تم نشر ${ok} تكليف، وفشل ${failed}`); router.refresh(); await onCreated?.(); resetForm(); }
    else toastError(firstError ? `تعذر إنشاء التكليف: ${firstError}` : 'تعذر إنشاء التكليف.');
  };

  const addLocation = async (label: string): Promise<string | void> => {
    try { const res = await fetch('/api/locations', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: label, type: 'OTHER' }) }); const d = await res.json().catch(() => ({})); if (res.ok && d?.location?.id) return d.location.id as string; toastError(d.error ?? `تعذر إضافة الجهة (${res.status}).`); } catch { toastError('تعذر الاتصال بالخادم أثناء إضافة المحكمة أو الجهة.'); }
  };
  ...
  return (
    <Modal open={open} onClose={onClose} title="تكليف" wide footer={
      <div className="flex flex-wrap items-center justify-between gap-2"><div className="text-[11px] font-bold text-navy-400">{busy ? `جارٍ النشر… (${progress}/${lawyerIds.length})` : createdCount > 0 ? `تم نشر ${createdCount} تكليف — يمكنك إضافة تكليف آخر` : 'كل إرسال ينشر تكليفاً مستقلاً'}</div><div className="flex gap-2">{createdCount > 0 && <Button variant="outline" onClick={resetForm}><RotateCcw size={14} /> تكليف آخر</Button>}<Button variant="ghost" onClick={onClose}>إغلاق</Button><Button type="submit" form="task-creator-form" disabled={busy}>{busy ? <Loader2 size={14} className="animate-spin" /> : <Plus size={15} />} نشر التكليف</Button></div></div>
    }>
      <form id="task-creator-form" onSubmit={submit} className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="المحكمة / جهة حكومية" required status={!!locationId}><ComboboxWithAdd id="task-location" name="locationId" ariaLabel="المحكمة أو الجهة الحكومية" placeholder="ابحث بالاسم أو اكتب محكمة أو جهة جديدة…" options={locations.map((l) => ({ value: l.id, label: l.name }))} value={locationId} onChange={setLocationId} onAdd={addLocation} /></Field>
        ...
        <Field label="التكليف — هيعمل إيه؟" status={!!description.trim()}><Textarea name="description" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="مثال: حضور جلسة الطعن رقم…" /></Field>
