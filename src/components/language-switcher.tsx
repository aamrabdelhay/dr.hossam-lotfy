...
export function LanguageSwitcher({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div className="flex items-center gap-1 rounded-full border border-white/70 bg-white/65 p-1 shadow-sm backdrop-blur-xl" aria-label="Language selector">
      ...
      {options.map((option) => (
        <button key={option.value} type="button" onClick={() => changeLanguage(option.value)} aria-pressed={language === option.value} title={option.label} className={`rounded-full px-2 py-1 text-[10px] font-extrabold transition ${language === option.value ? 'bg-navy-950 text-white shadow-sm' : 'text-navy-500 hover:bg-white hover:text-navy-950'} ${compact ? 'sm:px-1.5' : 'sm:px-2.5'}`}>
          {option.short}
        </button>
      ))}
    </div>
