...
export function LawyerProfileEditor({
  ...
  const openEditor = () => {
    setFullNameVal(fullName); setTitleVal(title); setPhoneVal(phone ?? ''); setEmailVal(email ?? '');
    setGoogleEmailVal(googleEmail ?? ''); setSpecializationVal(specialization ?? ''); setBioVal(bio ?? ''); setPositionVal(position ?? '');
    setOpen(true);
  };
  ...
  const save = async () => {
    ...
    try {
      const res = await fetch(`/api/lawyers/${lawyerId}`, {
        ...
        body: JSON.stringify({ fullName: fullNameVal.trim(), title: titleVal, phone: phoneVal.trim() || null, email: emailVal.trim() || null, googleEmail: googleEmailVal.trim() || null, specialization: specializationVal.trim() || null, bio: bioVal.trim() || null, position: positionVal.trim() || null }),
      });
      if (res.ok) { setOpen(false); toastSuccess('تم حفظ تعديلات صفحتك ✓'); router.refresh(); }
      else { const d = await res.json().catch(() => ({})); toastError(d.error ?? 'تعذر حفظ التعديلات.'); }
    } catch { toastError('تعذر الاتصال بالخادم. حاول مرة أخرى.'); } finally { setBusy(false); }
  };
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={`تعديل صفحتي — ${fullName}`} wide footer={<div className="flex justify-end gap-2"><Button variant="ghost" onClick={() => setOpen(false)}>إلغاء</Button><Button onClick={() => void save()} disabled={busy}>{busy && <Loader2 size={14} className="animate-spin" />}حفظ التعديلات</Button></div>}>
        <div className="space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="الهاتف" status={!!phoneVal.trim()}><Input value={phoneVal} onChange={(e) => setPhoneVal(e.target.value)} placeholder="01xxxxxxxxx" dir="ltr" /></Field>
            <Field label="البريد الإلكتروني" status={!!emailVal.trim()}><Input type="email" value={emailVal} onChange={(e) => setEmailVal(e.target.value)} dir="ltr" /></Field>
            <Field label="Google Email" status={!!googleEmailVal.trim()}><Input type="email" value={googleEmailVal} onChange={(e) => setGoogleEmailVal(e.target.value)} dir="ltr" /></Field>
            <Field label="المنصب" status={!!positionVal.trim()}><Input value={positionVal} onChange={(e) => setPositionVal(e.target.value)} placeholder="مثال: Senior Advocate" /></Field>
            <Field label="المجال / التخصص" status={!!specializationVal.trim()}><Input value={specializationVal} onChange={(e) => setSpecializationVal(e.target.value)} /></Field>
          </div>
          <Field label="النبذة التعريفية" status={!!bioVal.trim()}><Textarea value={bioVal} onChange={(e) => setBioVal(e.target.value)} className="min-h-[100px]" placeholder="نبذة قصيرة عن خبراتك ومجالات عملك…" /></Field>
        </div>
      </Modal>
    </>
  );
