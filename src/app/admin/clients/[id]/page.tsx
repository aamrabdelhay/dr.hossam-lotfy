...
import { ArrowRight, BriefcaseBusiness, CalendarDays, Clock3, FileText, Mail, MapPin, Phone, UserRound } from 'lucide-react';
...
export default async function ClientProfilePage({params}:{params:Promise<{id:string}>}){
  ...
  return <main className="mx-auto w-full max-w-[1440px] px-4 py-6 sm:px-6" dir="rtl">
    ...
    <div className="grid gap-5 lg:grid-cols-[1.25fr_.75fr]">
      <section className="space-y-4"><div className="flex items-center gap-2"><BriefcaseBusiness size={18} className="text-gold-700"/><h2 className="text-lg font-black text-navy-950">قضايا العميل</h2></div>{cases.length===0?<Card className="p-8 text-center text-sm font-bold text-navy-400">لا توجد قضايا مرتبطة بهذا العميل.</Card>:cases.map(c=><Card key={c.id} className="overflow-hidden p-0"><div className="border-b border-navy-100 p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-base font-black text-navy-900">{c.name}</h3><p className="mt-1 text-xs font-bold text-navy-400">رقم القضية: <span dir="ltr">{c.number}</span></p></div><div className="flex gap-2"><Badge text={`${c.eventCount} حدث`} /><Badge text={`${c.taskCount} تكليف`} /></div></div>{c.description&&<p className="mt-3 text-xs leading-6 text-navy-600">{c.description}</p>}<p className="mt-3 text-[11px] font-bold text-navy-400">بداية الأرشيف: {fmtDate(c.createdAt)}</p></div><div className="bg-navy-50/40 p-5"><div className="mb-3 flex items-center gap-2 text-xs font-black text-navy-700"><CalendarDays size={14}/> آخر أحداث القضية</div>{events.filter(e=>e.caseId===c.id).slice(0,5).map(e=><div key={e.id} className="relative border-s border-navy-200 pb-4 pe-4 last:pb-0"><span className="absolute -start-[5px] top-1 h-2 w-2 rounded-full bg-gold-500"/><p className="text-xs font-bold text-navy-800">{e.description}</p><p className="mt-1 text-[10px] text-navy-400">{fmtDate(e.createdAt)}{e.authorName?` · ${e.authorName}`:''}</p></div>)}{events.filter(e=>e.caseId===c.id).length===0&&<p className="text-xs font-semibold text-navy-400">لا يوجد أرشيف مسجل لهذه القضية حتى الآن.</p>}</div></Card>)}</section>
      <section className="space-y-4"><div className="flex items-center gap-2"><Clock3 size={18} className="text-gold-700"/><h2 className="text-lg font-black text-navy-950">السجل الزمني والتكليفات</h2></div><Card className="p-5"><div className="mb-4 text-xs font-black text-navy-700">كل حركة مرتبطة بقضايا العميل</div>{tasks.length===0?<p className="text-xs font-semibold text-navy-400">لا توجد تكليفات أو جلسات مسجلة.</p>:tasks.map(t=><div key={t.id} className="border-b border-navy-100 py-4 last:border-0 last:pb-0"><div className="flex items-start justify-between gap-3"><p className="text-xs font-extrabold text-navy-800">{t.description}</p><span className="shrink-0 rounded-full bg-navy-100 px-2 py-1 text-[9px] font-black text-navy-600">{statusLabel(t.status)}</span></div><p className="mt-2 text-[10px] font-semibold text-navy-400">{caseNames.get(t.caseId??'')||'قضية'} · {fmtDate(t.scheduledDate)}{t.scheduledTime?` · ${t.scheduledTime}`:''}{t.locationName?` · ${t.locationName}`:''}</p>{t.notes&&<p className="mt-2 text-[11px] leading-5 text-navy-500">{t.notes}</p>}</div>)}</Card></section>
    </div>
    <div className="mt-5 text-center text-[10px] font-semibold text-navy-400">تم إنشاء ملف العميل في {fmtDate(client.createdAt)} · آخر تحديث {fmtDate(client.updatedAt)}</div>
  </main>
}
function Info({icon,label,value}:{icon:React.ReactNode;label:string;value:string|null}){return <div className="rounded-2xl border border-navy-100 bg-navy-50/40 p-3"><div className="flex items-center gap-2 text-[10px] font-bold text-navy-400">{icon}{label}</div><div className="mt-1 break-words text-xs font-extrabold text-navy-800">{value||'غير مسجل'}</div></div>}
function Badge({text}:{text:string}){return <span className="rounded-full bg-gold-500/10 px-2.5 py-1 text-[9px] font-black text-gold-700">{text}</span>}
