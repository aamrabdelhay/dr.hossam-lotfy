...
const createSchema = z.object({
  ...
  title: z.enum(['DOCTOR', 'ADVOCATE'], {
    message: 'اختر الصفة: دكتور أو محامي',
  }),

  phone: z
    .string()
    .min(6, 'رقم التليفون مطلوب')
    .max(20),
...
export const POST = handle(async (req: Request) => {
  ...
  if (parts.length < 3) {
    return json(
      {
        error:
          'الاسم يجب أن يكون ثلاثياً على الأقل: (أول، وسط، عائلة)',
      },
      { status: 400 }
    );
  }
  ...
  const lawyer = await prisma.lawyer.create({
    data: {
      ...
      email: data.email,
      googleEmail:
        data.googleEmail?.trim().toLowerCase() || null,
      specialization:
