...
export const POST = handle(async (req: Request) => {
  ...
  const when = task.scheduledDate ? `${formatDay(task.scheduledDate)}${task.scheduledTime ? ` — ${task.scheduledTime}` : ''}` : 'بالتنسيق'; await logActivity({ action: 'CREATED', summary: `أنشأ مهمة جديدة: ${(task.description || data.locationId || '').slice(0, 80)}`, taskId: task.id, locationId: data.locationId, byUserId: session.role === 'admin' ? session.userId : null, byLawyerId: isOwnPost ? session.lawyerId : null }).catch(() => undefined); await notifyTaskAssigned(lawyerIds, task.description.slice(0, 60), when, `/sessions/${task.id}`).catch(() => undefined); if (isOwnPost) await notifyPostCreated(task.id, session.name, task.description.slice(0, 60), task.location?.name ?? '').catch(() => undefined);
