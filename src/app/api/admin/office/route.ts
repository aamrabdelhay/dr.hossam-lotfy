...
export async function GET() {
  ...
  const [members, users, lawyers, requests, logins, clicks, archive, categories, dues] = await Promise.all([
    ...
    prisma.$queryRawUnsafe<any[]>(`SELECT * FROM "office_archive" ORDER BY "deleted_at" DESC LIMIT 500`),
...
export async function POST(req: Request) {
  ...
  if (body.action === 'restore') {
    ...
    else if(entry.entity_type==='task') await prisma.$executeRawUnsafe(`UPDATE "tasks" SET "status"='PENDING',"updatedAt"=NOW() WHERE "id"=$1`,entry.entity_id);
