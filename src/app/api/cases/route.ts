...
const createSchema = z.object({ name: z.string().min(1, 'اسم القضية مطلوب').max(300), number: z.string().min(1, 'رقم القضية مطلوب').max(200), description: z.string().max(1000).optional(), clientName: z.string().max(200).optional() });

export const POST = handle(async (req: Request) => {
  await requirePermission('writeTasks');
  const data = await readJson(req as never, createSchema);
  const demo = await isDemoMode();
  const c = await prisma.caseRecord.create({ data: { name: demo ? `${data.name.trim()} ${DEMO_TAG}` : data.name.trim(), number: demo ? `DEMO-${data.number.trim()}` : data.number.trim(), description: data.description?.trim() || null, clientName: data.clientName?.trim() || null } });
