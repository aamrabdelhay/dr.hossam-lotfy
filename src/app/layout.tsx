...
export const viewport: Viewport = { themeColor: [{ media: '(prefers-color-scheme: dark)', color: '#111513' }, { media: '(prefers-color-scheme: light)', color: '#f5f4ef' }], width: 'device-width', initialScale: 1 };
...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  const themeScript = `(function(){try{var t=localStorage.getItem('hl-theme');if(t!=='light'&&t!=='dark'){t=window.matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light';}document.documentElement.setAttribute('data-theme',t);}catch(e){document.documentElement.setAttribute('data-theme','light');}})();`;
  return <html dir="rtl" lang="ar" data-theme="light" suppressHydrationWarning><head><script dangerouslySetInnerHTML={{ __html: themeScript }} /></head><body className="flex min-h-screen flex-col"><Navbar lawyers={nav.lawyers} locations={nav.locations} session={session ? { role: session.role, name: session.name, slug: session.role === 'lawyer' ? session.slug : undefined, photo: null, isAdmin: session.role === 'admin' || (session.role === 'lawyer' && session.isAdmin) } : null} unread={unread} />{databaseUnavailable && <div role="alert" className="border-b border-[var(--danger-solid)] bg-[var(--danger-solid)] px-4 py-2.5 text-center text-[12px] text-white">قاعدة البيانات غير متاحة حالياً.{' '}<Link href="/api/health" className="underline decoration-white/60 underline-offset-4 hover:text-white/90">عرض تشخيص الخدمة</Link></div>}<WarningBar active={sidebar.warning} count={sidebar.warningCount} /><main className="flex-1">{children}<LawyerGuideEnhancements /></main><MobileSessions data={sidebar} isAdmin={session?.role === 'admin' || (session?.role === 'lawyer' && session.isAdmin)} /><Footer /><Toaster /></body></html>;
}
