...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  if (session) {
    ...
    const now = new Date();
    const unreadResult = await loadFailSoft('notification count', () => prisma.notification.count({ where: { ...owner, readAt: null, OR: [{ type: { not: 'PERSONAL_REMINDER' } }, { type: 'PERSONAL_REMINDER', createdAt: { lte: now } }] } }), 0);
    unread = unreadResult.value; databaseUnavailable ||= unreadResult.unavailable;
  }
  return <html dir="rtl" lang="ar"><body className="flex min-h-screen flex-col">
    <Navbar lawyers={nav.lawyers} locations={nav.locations} session={session ? { role: session.role, name: session.name, slug: session.role === 'lawyer' ? session.slug : undefined, photo: null } : null} unread={unread} />
    ...
    <MobileSessions data={sidebar} isAdmin={session?.role === 'admin'} /><Footer /><Toaster />
  </body></html>;
}
