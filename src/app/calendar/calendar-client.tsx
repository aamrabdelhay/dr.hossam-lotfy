    ...
    <div>
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
        ...
        <div className="flex w-full flex-wrap items-center gap-2 sm:ms-auto sm:w-auto">
          <div className="flex rounded-lg border border-navy-200 bg-white p-0.5">{(['month', 'week', 'day'] as View[]).map((v) => <button key={v} onClick={() => setView(v)} className={cn('rounded-md px-3 py-1.5 text-[12px] font-extrabold transition', view === v ? 'bg-navy-950 text-gold-300' : 'text-navy-400 hover:text-navy-800')}>{v === 'month' ? UI('month') : v === 'week' ? UI('week') : UI('day')}</button>)}</div>
          <div className="flex items-center gap-1"><Button variant="outline" size="sm" onClick={() => shift(-1)} aria-label={UI("previous")}><ChevronRight size={14} /></Button><button onClick={() => { const d = new Date(); d.setHours(0, 0, 0, 0); setCursor(d); }} className="rounded-md px-2 text-[12px] font-bold text-navy-500 hover:text-navy-900">{UI("today")}</button><Button variant="outline" size="sm" onClick={() => shift(1)} aria-label={UI("next")}><ChevronLeft size={14} /></Button></div>
          ...
          {isAdmin && <Button variant="gold" size="sm" onClick={() => { const p = (n: number) => String(n).padStart(2, '0'); setSelectedDate(`${cursor.getFullYear()}-${p(cursor.getMonth() + 1)}-${p(cursor.getDate())}`); setCreatorOpen(true); }}><CalendarPlus size={14} />{UI("add")}</Button>}
        </div>
      </div>
