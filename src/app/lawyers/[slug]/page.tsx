...
function cleanDemo(text: string | null | undefined): string { if (!text) return ''; return text.replace(/\(تجريبي\)/g, '').replace(/\(تجريبية\)/g, '').replace(/\s{2,}/g, ' ').trim(); }
function getFeminineTitle(fullName: string, title: string): string { const lower = fullName.toLowerCase(); if (lower.includes('mona') || lower.includes('sara') || fullName.includes('مونا') || fullName.includes('سارا')) return 'محامية'; return TITLE_LABEL[title] ?? title; }
        ...
        <header className="mb-8 rounded-3xl border border-navy-100 bg-white px-5 py-7 text-center shadow-card sm:px-8 sm:py-9">
          <div className="flex flex-col items-center gap-5">
            <div className="min-w-0">
              <p className="mb-2 text-[10px] font-bold uppercase tracking-[2px] text-gold-600" style={{ fontFamily: 'Inter, sans-serif' }}>LAWYER PROFILE</p>
              <h1 className="text-3xl font-bold leading-tight text-navy-950 sm:text-4xl" style={{ fontFamily: 'Inter, Cairo, sans-serif' }}>{lawyer.fullName}</h1>
              {(isSelf || isAdminSession) && lawyer.email && <p className="mt-2 text-sm text-navy-400" dir="ltr" style={{ fontFamily: 'Inter, sans-serif' }}>{lawyer.email}</p>}
              <p className="mt-3 text-sm text-navy-500" style={{ fontFamily: 'Cairo, sans-serif' }}>{getFeminineTitle(lawyer.fullName, lawyer.title)} · {specialization}</p>
            </div>
