...
export default async function SessionsPage() {
  const upcoming = await getFeed({ from: new Date().toISOString().slice(0, 10), limit: 50 });
  const past = await getFeed({ to: new Date(Date.now() - 86400000).toISOString().slice(0, 10), limit: 25 });
    ...
    <div className="mx-auto w-full max-w-[1000px] px-4 py-6 sm:px-6">
      <div className="mb-6">
        ...
        <p className="mt-1 text-[13px] font-medium text-navy-400">
          جدول كل الجلسات القادمة مرتباً زمنياً، يليه سجل الجلسات السابقة.
        </p>
      </div>
      ...
      <section className="mb-10">
        <h2 className="mb-3 flex items-center gap-2 text-[14px] font-extrabold text-navy-800">
          ...
          القادمة
          <span className="text-[11px] font-bold text-navy-300">({upcoming.total})</span>
        </h2>
              ...
              <Link key={t.id} href={`/sessions/${t.id}`} className="block">
                <Card className="p-4 transition hover:border-gold-500 hover:shadow-md">
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5">
                    <span className="flex items-center gap-1.5 text-[13px] font-extrabold text-navy-900">
                      <CalendarClock size={14} className="text-gold-600" />
                      {t.scheduledDate ?? 'بالتنسيق'}
                      {t.scheduledTime && (
                        <span className="flex items-center gap-1 text-[12px] font-bold text-navy-400">
                          <Clock3 size={12} />
                          <span className="font-latin">{t.scheduledTime}</span>
                        </span>
                      )}
                    </span>
      ...
      {past.items.length > 0 && (
        <section>
          <h2 className="mb-3 text-[14px] font-extrabold text-navy-800">
            سابقة
            <span className="ms-1.5 text-[11px] font-bold text-navy-300">({past.total})</span>
          </h2>
