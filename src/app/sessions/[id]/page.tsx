      ...
      <div className="min-w-0 flex-1 space-y-5">
        <Card className="overflow-hidden">
          ...
          <div className="grid gap-x-6 gap-y-4 px-5 py-5 sm:grid-cols-2 lg:grid-cols-3">
            <Detail icon={<MapPin size={15} />} label="المكان"><Link href={`/locations/${task.location.slug}`} className="font-extrabold text-ivory-100 hover:underline">{task.location.name}</Link><span className="text-[11px] font-bold text-navy-300">{LOCATION_TYPE_LABEL[task.location.type]}</span></Detail>
            <Detail icon={<CalendarDays size={15} />} label="التاريخ واليوم">{date ? <span className="font-extrabold text-ivory-100">{formatFullDate(date)}</span> : 'غير محدد'}</Detail>
            <Detail icon={<Clock size={15} />} label="الساعة">{time ? <span className="font-extrabold text-ivory-100">{time}</span> : 'غير محدد'}</Detail>
            {(task.caseName || task.caseNumber) && <Detail icon={<FileText size={15} />} label="القضية"><span className="font-extrabold text-ivory-100">{task.caseName}</span><span className="font-latin text-[11px] font-bold text-navy-400">{task.caseNumber}</span></Detail>}
            ...
            <Detail icon={<PlusCircle size={15} />} label="الإضافة"><span className="text-[12.5px] font-bold text-ivory-300">{task.author ? `${task.author.name} (${TITLE_LABEL[task.author.title]})` : raw.createdBy ? `إدارة — ${raw.createdBy.name}` : 'إدارة المكتب'}</span><span className="text-[11px] font-semibold text-navy-300">{formatDateTime(raw.createdAt)}</span></Detail>
          </div>
          {task.notes && <div className="mx-5 mb-5 flex items-start gap-2 rounded-lg border border-gold-500/25 bg-gold-500/[0.06] px-3 py-2.5"><StickyNote size={14} className="mt-1 shrink-0 text-gold-500" /><div><p className="text-[11px] font-extrabold text-gold-500">ملاحظات</p><p className="mt-0.5 text-[13px] leading-6 text-ivory-300">{task.notes}</p></div></div>}
        </Card>
        ...
        <div><h2 className="mb-3 text-[15px] font-extrabold text-ivory-100">سجل النشاط على هذه المهمة</h2>{taskActivity.length === 0 ? <EmptyState title="لم يتم تسجيل أي نشاط" hint="تظهر هنا كل الأحداث: إنشاء، تعديل، إسناد، تنفيذ، تعليقات، حذف." /> : <Card className="p-5"><ActivityTimeline items={taskActivity} /></Card>}</div>
      </div>
    </div>
  );
}
...
function Detail({ icon, label, children }: { icon: React.ReactNode; label: string; children: React.ReactNode }) {
  return <div><p className="mb-1 flex items-center gap-1.5 text-[11px] font-extrabold text-navy-400"><span className="text-gold-500">{icon}</span>{label}</p><div className="text-[13px] leading-6">{children}</div></div>;
}
